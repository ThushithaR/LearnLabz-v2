...
import { Calculator, X, CheckCircle, XCircle, ChevronDown, ChevronRight, Sun, ArrowLeft, AlertCircle } from "lucide-react";
...
export default function BFSTreeTraversal() {
    ...
    <div className="fixed inset-0 z-50 bg-background flex flex-col animate-in fade-in duration-300 overflow-y-auto lg:overflow-hidden">
      ...
      <div className="bg-surface border-b border-white/5 px-4 md:px-6 py-3 flex items-center justify-between shrink-0 h-16 sticky top-0 z-20 backdrop-blur-md bg-surface/80">
        <div>
          ...
          <p className="text-sm text-textSecondary">Breadth-First Search Challenge • Find Node 11</p>
        </div>
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-12 h-fit lg:h-full">
            ...
            <div className="w-full lg:col-span-6 bg-[#0F0E0D] relative flex flex-col min-h-[50vh] lg:min-h-full overflow-auto">
              ...
              <div className="flex-1 p-8 overflow-auto">
                <div className="mb-6">
                  <h3 className="text-sm font-bold text-accent mb-4">Interactive Tree (Click ANY node to try adding to queue)</h3>
                ...
                <div className="bg-surface/30 rounded-xl border border-white/5 p-6 mb-4">
                  <h3 className="text-sm font-bold text-textSecondary mb-3">QUEUE (FIFO)</h3>
                  <div className="flex items-center gap-2 flex-wrap">
                    {queue.length === 0 ? (
                      <span className="text-textSecondary text-sm">Empty</span>
                    ) : (
                      queue.map((node, idx) => (
                        <div
                          key={idx}
                          className={cn(
                            "px-4 py-2 rounded-lg font-mono font-bold border-2",
                            idx === 0 ? "bg-accent/20 border-accent text-accent" : "bg-white/5 border-white/10 text-white"
                          )}
                        >
                          {node}
                ...
                <div className="bg-surface/30 rounded-xl border border-white/5 p-6 mb-4">
                  <h3 className="text-sm font-bold text-textSecondary mb-3">YOUR TRAVERSAL ORDER</h3>
                  <div className="flex items-center gap-2 flex-wrap">
                    {userTraversalOrder.length === 0 ? (
                      <span className="text-textSecondary text-sm">No nodes selected yet</span>
                    ) : (
                ...
                {/* Error Message */}
                {errorMessage && (
                  <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 mb-4 flex items-start gap-3 animate-in slide-in-from-top-2">
                ...
                <div className="bg-accent/5 rounded-xl border border-accent/20 p-4 mb-4">
                  <h3 className="text-sm font-bold text-accent mb-2">Current Front Node: {currentFront}</h3>
