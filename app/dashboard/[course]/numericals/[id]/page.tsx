...
export default function NumericalsSolvePage({ params }: { params: { id: string } }) {
    ...
    const handleExit = () => {
        ...
        router.push('/dashboard/numericals');
    };
    ...
    return (
        <div className="fixed inset-0 z-50 bg-background flex flex-col animate-in fade-in duration-300 overflow-y-auto lg:overflow-hidden">
            ...
            <div className="bg-surface border-b border-white/5 px-4 md:px-6 py-3 flex items-center justify-between shrink-0 h-16 sticky top-0 z-20 backdrop-blur-md bg-surface/80">
                <div>
                    <h1 className="text-lg font-bold text-textPrimary">Numerical Challenge #{params.id}</h1>
                    <div className="text-xs text-textSecondary hidden md:block">Minimax Algorithm • Game Theory</div>
                </div>
            ...
            <div className="flex-1 flex flex-col lg:grid lg:grid-cols-12 overflow-hidden">
                ...
                <div className="w-full lg:col-span-3 bg-surface/30 border-r border-white/5 p-6 overflow-y-auto max-h-[40vh] lg:max-h-full shrink-0">
                    ...
                    <div className="p-4 bg-accent/5 rounded-lg border border-accent/10">
                        ...
                        <p className="text-xs text-textSecondary">Remember that the root is a Maximizer, so it will choose the child with the highest value.</p>
                    </div>
                </div>
                ...
                <div className="w-full lg:col-span-3 bg-surface/30 border-t lg:border-t-0 lg:border-l border-white/5 p-6 flex flex-col shrink-0">
                    ...
                    <div className="space-y-6 mb-8 lg:mb-auto">
                        <div>
                            ...
                            <input type="number" className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white outline-none focus:border-accent text-lg font-mono transition-all focus:ring-1 ring-accent/50" placeholder="?" />
                        </div>
                        <div>
                            <label className="text-xs font-medium text-textSecondary mb-2 block">Nodes Pruned</label>
                            <input type="number" className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white outline-none focus:border-accent text-lg font-mono transition-all focus:ring-1 ring-accent/50" placeholder="0" />
                        </div>
                    </div>

                    <Button size="lg" className="w-full gap-2 font-bold py-6 text-base shadow-lg shadow-accent/20" onClick={handleExit}>
                        Submit Solution
                    </Button>
                </div>
            </div>
