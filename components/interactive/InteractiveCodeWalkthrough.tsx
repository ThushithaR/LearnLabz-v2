    ...
    <div ref={containerRef} className="w-full max-w-6xl mx-auto bg-transparent relative my-8 font-sans">
                    ...
                    <div className={`whitespace-pre-wrap break-all w-full ${isActive ? "" : "text-gray-600 opacity-60"}`}>
                      {ln.map((tok, ti) => (
                        <span
                          ...
                          className={`whitespace-pre-wrap ${isActive ? (tok.cls ?? "text-white") : ""} ${isActive ? "hover:bg-white/10 cursor-help rounded px-0.5" : ""}`}
                        >
                          {tok.text}
                        </span>
                      ))}
      ...
      {hover && isVisible && (
        <div
          ...
          className="pointer-events-none absolute z-50 bg-[#1a1a1a] text-white border border-white/20 text p-3 rounded-lg shadow-2xl max-w-[280px] animate-in fade-in zoom-in-95 backdrop-blur-md"
        >
