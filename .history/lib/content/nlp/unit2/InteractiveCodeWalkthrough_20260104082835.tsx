    ...
    <div ref={containerRef} className="w-full max-w-4xl mx-auto bg-transparent relative">
      ...
      {hover && (
        <div
          ...
          className="pointer-events-none absolute z-50 bg-black/90 text-white border border-white/20 text-sm p-3 rounded shadow-lg max-w-sm break-words"
        >
          <div className="font-medium">{hover.text}</div>
          {hover.note && <div className="text-white/80 text-sm mt-1">{hover.note}</div>}
        </div>
      )}
    </div>
  );
