...
type Token = { text: string; note?: string; cls?: string };
...
const outputs: string[] = ["", "", "", "15"]; // only last line prints

export default function InteractiveCodeWalkthrough() {
  ...
  const visibleOutputs = outputs.slice(0, index).filter(Boolean);

  return (
    <div ref={containerRef} className="w-full max-w-4xl mx-auto bg-transparent">
      <div className="flex gap-6">
        {/* Left: Code */}
        <div className="w-1/2 bg-surface p-4 rounded-lg border border-white/10 font-mono text-sm">
          <div className="space-y-2">
            {lines.slice(0, index).map((ln, li) => (
              <div key={li} className="flex items-start gap-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {ln.map((tok, ti) => (
                    <span
                      key={ti}
                      onMouseEnter={(e) => onTokenEnter(e, tok)}
                      onMouseLeave={onTokenLeave}
                      className={`px-1 py-0.5 rounded ${tok.cls ?? "text-textPrimary"} hover:bg-white/5 cursor-help`}
                    >
                      {tok.text}
                    </span>
                  ))}
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <button onClick={next} disabled={index === lines.length} className="px-3 py-1 rounded bg-accent">Next</button>
        ...
        {/* Right: Output */}
        <div className="w-1/2 bg-black/80 p-4 rounded-lg border border-white/10 text-sm text-white">
          <div className="mb-2 text-textSecondary">Output</div>
          <div className="h-40 overflow-auto rounded bg-black/70 p-3">
          ...
          {finished && (
            <div className="mt-4 p-3 bg-white/5 rounded">
              ...
              <div className="text-sm text-textSecondary">
                This code declares two integers `a` and `b`, computes `c` as their sum, and prints the result to standard output. The final printed value is <strong className="text-white">15</strong>.
              </div>
            </div>
          )}
        </div>
      ...
      {/* Hover Tooltip */}
      {hover && (
        <div style={{ left: hover.x, top: hover.y }} className="pointer-events-none absolute z-50 bg-surface/90 border border-white/10 text-xs p-2 rounded shadow-sm">
          <div className="font-medium">{hover.text}</div>
          {hover.note && <div className="text-textSecondary text-xs mt-1">{hover.note}</div>}
        </div>
      )}
    </div>
  );
