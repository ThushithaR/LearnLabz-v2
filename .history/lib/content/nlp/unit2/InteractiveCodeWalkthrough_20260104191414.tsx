                ...
                <div key={li} className="flex items-start gap-4 group">
                  ...
                  <div className="flex flex-wrap gap-x-2 gap-y-1">
                    {ln.map((tok, ti) => {
                      ...
                      const lineClass = isPrevLine ? "opacity-40 text-white/40" : "text-textPrimary";
                      return (
                        <span
                          key={ti}
                          onMouseEnter={hoverEnabled ? (e) => onTokenEnter(e, tok) : undefined}
                          onMouseLeave={isActiveLine ? () => setHover(null): undefined}
                          className={`px-1 rounded transition-colors whitespace-pre-wrap ${tok.cls ?? "text-textPrimary"} ${hoverEnabled ? "hover:bg-white/10 cursor-help" : "opacity-60"}`}
                          style={{`px-1 rounded transition-colors whitespace-pre-wrap ${lineClass} ${hoverEnabled ? "hover:bg-white/10 cursor-help" : ""}`}}  // Alternatively, use this inline style
                        >
                          {tok.text}
                        </span>
                      );
      ...
      {hover && (
       <div
          ...
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded-full bg-accent/20 flex items-center justify-center text-xs">?</div>
            <div className="font-medium text-accent">{hover.text}</div>
          </div>
          {hover.note && <div className="text-white/80 text-sm mt-2 leading-relaxed">{hover.note}</div>}
        </div>
      )}
