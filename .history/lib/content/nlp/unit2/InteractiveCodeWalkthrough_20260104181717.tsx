...
export default function InteractiveCodeWalkthrough({ lines, outputs, summary, enablePrevHover = false }: InteractiveProps) {
  ...
  return (
    <div ref={containerRef} className="w-full max-w-4xl mx-auto bg-transparent relative">
      ...
      <div className={`${codeMinimized ? 'flex' : 'flex gap-6'}`}>
        {/* Left: Code */}
        {!codeMinimized && (
        ...
        {/* Right: Output */}
        <div className={`${codeMinimized ? 'w-full' : 'w-2/5'} bg-black/80 p-4 rounded-lg border border-white/10 text-sm text-white flex flex-col overflow-hidden`}>
