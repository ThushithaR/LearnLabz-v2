    ...
    <div ref={containerRef} className="w-full max-w-6xl mx-auto bg-transparent relative my-8">
      ...
      <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4">
        <div className="flex items-center gap-4">
          ...
          >
            {isVisible ? 'Hide Source' : 'Show Source'}
          </button>
        </div>
        {isVisible && (
           <div className="text-[10px] text-textSecondary uppercase tracking-widest">
      ...
      {isVisible && (
        <div className="flex gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div className="w-[60%] bg-[#0d1117] p-5 rounded-xl border border-white/10 font-mono text-sm shadow-2xl">
            <div className="space-y-3">
              ...
              <div className="mt-8 pt-4 border-t border-white/5 flex items-center gap-3">
                ...
                <button onClick={handleFinish} className="ml-auto px-4 py-1.5 rounded border border-emerald-500/50 text-emerald-400 hover:bg-emerald-500/10 text-xs transition-all">EXECUTE ALL</button>
              </div>
            </div>
          </div>
          ...
          <div className="w-[40%] flex flex-col gap-4">
            ...
            {finished && (
              <div className="p-5 bg-accent/5 rounded-xl border border-accent/20 animate-in zoom-in-95 duration-300">
                <div className="text-[10px] uppercase tracking-widest text-accent mb-2 font-bold">Analysis Summary</div>
      ...
      {hover && isVisible && (
        <div
          ...
          className="pointer-events-none absolute z-50 bg-gray-900 text-white border border-white/20 text-xs p-3 rounded-lg shadow-2xl max-w-[250px] animate-in fade-in zoom-in-95"
        >
          <div className="font-bold text-accent mb-1 border-b border-white/10 pb-1">{hover.text}</div>
